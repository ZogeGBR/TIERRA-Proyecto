...
export default function CarritoPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-6 py-16 text-center">
        ...
        <Link href="/productos" className="mt-4 inline-block text-tierra-azul underline">
          Ver catálogo
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
      <h1 className="text-2xl font-medium text-tierra-bordo-oscuro mb-6">Carrito de compras</h1>
      ...
      <div className="bg-white border border-tierra-crema-oscuro rounded-lg divide-y divide-tierra-crema-oscuro">
        {items.map((item) => (
          <div key={item.varianteId} className="flex items-center gap-4 p-4">
            ...
            <button onClick={() => quitarItem(item.varianteId)} className="text-tierra-bordo/50 text-sm">
              Quitar
            </button>
          </div>
        ))}
      ...
      <div className="mt-6 flex justify-between items-center">
        ...
        <Link href="/checkout" className="bg-tierra-naranja text-white px-6 py-3 rounded-full font-medium">
          Continuar compra
        </Link>
      </div>
    </div>
